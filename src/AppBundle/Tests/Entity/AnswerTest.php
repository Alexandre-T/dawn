...
class AnswerTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * Tests Answer get add removeScene ().
     */
    public function testScenes()
    {
        $scene1 = new Scene();
        $scene2 = new Scene();
        $this->answer->addScene($scene1);
        self::assertCount(1, $this->answer->getScenes());
        self::assertTrue($this->answer->getScenes()->contains($scene1));
        self::assertFalse($this->answer->getScenes()->contains($scene2));
        $this->answer->addScene($scene2);
        self::assertCount(2, $this->answer->getScenes());
        self::assertTrue($this->answer->getScenes()->contains($scene1));
        self::assertTrue($this->answer->getScenes()->contains($scene2));
        $this->answer->removeScene($scene1);
        self::assertCount(1, $this->answer->getScenes());
        self::assertFalse($this->answer->getScenes()->contains($scene1));
        self::assertTrue($this->answer->getScenes()->contains($scene2));
        $this->answer->removeScene($scene2);
        self::assertCount(0, $this->answer->getScenes());
        self::assertFalse($this->answer->getScenes()->contains($scene1));
        self::assertFalse($this->answer->getScenes()->contains($scene2));
    }
}
